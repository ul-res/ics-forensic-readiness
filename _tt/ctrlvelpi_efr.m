function [exectime, data] = ctrlvelpi_efr(segment, data)
%CTRLVELPI_EFR Kernel implementation of the E feed rate controller.
%Standard velocity PI controller.
%
% INPUTS
%   - net message from sensor xmeas3
%   - analog in for setpoint (r3*fp)
%
% OUTPUTS
%   - net message with xmv2 to actuator + to EWS.
%   - net message with error(17) to EWS
...
switch segment
    case 1 % handle incoming message & compute setpoint - measurement error
        ...
        % Send control signal xmv2 over specified channel to actuator + to EWS.
        msg = struct('timestamp', ttCurrentTime, 'data', data.PreviousUsignal, 'destination', [3 8; 4 5], 'order', 2);
        ttSendMsg([2 1], msg, 160);
        % Send current error to the EWS for the look-ahead initialisation.
        msg = struct('timestamp', ttCurrentTime, 'data', data.PreviousError, 'destination', [4 5], 'order', 38);
        ttSendMsg([2 1], msg, 160);
